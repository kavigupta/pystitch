...
from ordering import order_replacements_map
...
class ProductionFactory:
    ...
    def add_programs(self, programs: List[str], canonicalize=True):
        for program in programs:
            self.add_program(program, canonicalize=canonicalize)

    def add_solution(self, program: List[str]):
        assert isinstance(program, str)
        self.add_program(program, is_solution=True)

    def add_program(self, s_exprs: str, is_solution=False, canonicalize=True):
    ...
    def make_param_var(self, ast, production: AbstractionProduction, idx: int):
        ...
        if body.startswith("&"):
            ...
            return
        name = f"fn_param_{len(self.params)}"
        production = ParamProduction(name, ast, production.arg_dfa_root(idx))
        self.params[body] = production
        self._code_to_params[name] = production
    ...
    def rewrite_solutions(self, config=Config()):
        solutions = [p for p in self.programs.values() if p.is_solution]
        if not solutions:
            return None
        return [self.rewrite_programs(sol, sol.ast, config) for sol in solutions]

    def expand_dfa(self, dfa):
        dfa = dfa.copy()
        for name, param in self._code_to_params.items():
            root = param.dfa_root
            children = [
