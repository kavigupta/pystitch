...
from imperative_stitch.analyze_program.ssa.ivm import is_argument
from imperative_stitch.utils.ast_utils import ReplaceNodes, ast_nodes_in_order


def variables_needed_to_extract(scope_info, extract_node, node_to_ssa, ssa_to_origins):
    """
    ...
    for node in node_list:
        ...
        if node in node_to_ssa:
            ...
            if is_argument(ssa, ssa_to_origins):
                continue
...
def extract_as_function(
    scope_info, metavariable_node, node_to_ssa, ssa_to_origins, metavariable_name
):
    """
    ...
    variables = variables_needed_to_extract(
        scope_info, metavariable_node, node_to_ssa, ssa_to_origins
    )
    args = ast.arguments(
...
def extract_metavariables(scope_info, site, node_to_ssa, ssa_to_origins):
    """
