...
class CheckSamplesValid(unittest.TestCase):
    def check_samples(self, *programs, count=100):
        ...
        dfa = export_dfa(TRANSITIONS)
        fam, dist = fit_to(programs)
        samples = fam.sample(dist, count, np.random.RandomState(0))
        samples = [ns.render_s_expression(x) for x in samples]
        for samp in samples:
            py_code = ParsedAST.parse_s_expression(samp).to_python()
            print(samp)
            samp_reconstructed = ParsedAST.parse_python_module(
                py_code
            ).to_type_annotated_ns_s_exp(dfa)
